...
8. `PhaseToRangeFrom0To2PI` - перенос значения фазы в пределы [0, 2 * M_PI). Сложность: $`O(phase / (2 * M_PI))`$;
...
Если возникает потребность в подключении библиотеки без возможности её редактирования, следует выполнить
следующие действия

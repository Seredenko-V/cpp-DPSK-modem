...
Сложность: $`O(N)`$;
4. `DetermineUsedCarrierFreq` - определить значение используемой несущей частоты. Сложность: $`O(1)`$. Если пользователем была задана промежуточная
частота, то будет использована она, если пользователь предварительно не указал значение промежуточной частоты, то она будет выбрана автоматически;
5. `ExtractSymbolsFromBits` - конвертировать биты в символы в соответствии с позиционностью ОФМ и наличием опорного символа в последовательности бит.
Сложность: $`O(N)`$;
...
должен поступать сигнал, для которого установлена тактовая синхронизация.
  ...
  Главный функционал реализован в методе `Demodulation`, который принимает на вход последовательность отсчетов в формате `const std::vector<double>&`
и возвращает последовательность символов в 10-й системе счисления в виде `std::vector<uint32_t>`. Сложность метода - $`O(N)`$.

  Данный метод также имеет перегрузку, которая принимает последовательность синфазных и квадратурных составляющих в формате
`const std::vector<std::complex<double>>&` и возвращает последовательность символов в 10-й системе счисления в виде `std::vector<uint32_t>`.
Сложность перегрузки метода также $`O(N)`$.
